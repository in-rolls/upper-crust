...
Notebook-friendly
  • Use collect_run_notebook(...) with a list of region dicts.
  • CLI still works; unknown Jupyter flags are ignored.
    ...
    def from_city(
        ...
        *,
        ...
        if gmaps_client is None:
            raise ValueError("City geocoding requires googlemaps; pass --google-api-key or explicit center")
        ge = gmaps_client.geocode(city)
        if not ge:
            raise ValueError(f"Could not geocode city: {city}")
        loc = ge[0]["geometry"]["location"]
        return cls(region_id=region_id, region_name=region_name or city,
                   center_lat=float(loc["lat"]), center_lon=float(loc["lng"]),
                   radius_km=radius_km, grid_step_km=grid_step_km)
...
def collect_places_for_region(
    ...
    *,
    ...
    for idx, (lat, lon) in enumerate(
        grid_cells(region.center_lat, region.center_lon, region.radius_km, region.grid_step_km)
    ):
        ...
        for lang in languages:
            try:
                if verbose:
                    print(f"· {region.region_id}: cell={idx} lang={lang} near=({lat:.4f},{lon:.4f})")
                page_idx = 0
                result = gmaps_client.places_nearby(
                    location=(lat, lon), radius=places_radius_m, type="restaurant", language=lang
                )
                meta["requests_made"] += 1
                for r in result.get("results", []):
                    pid = r.get("place_id"); nm = (r.get("name") or "").strip()
                    geom = (r.get("geometry") or {}).get("location") or {}
                    plat = geom.get("lat"); plon = geom.get("lng")
                ...
                while "next_page_token" in result:
                    ...
                    result = gmaps_client.places_nearby(page_token=result["next_page_token"])
                    meta["requests_made"] += 1
                    meta["pages_seen"] += 1
                    page_idx += 1
                    for r in result.get("results", []):
    ...
    if verbose:
        print(f"✅ Places[{region.region_id}]: {len(out)} unique | cells={meta['cells_covered']} langs={len(languages)} requests={meta['requests_made']} pages={meta['pages_seen']}")
    return out, meta
...
def collect_overpass_for_region(
    ...
    *,
    ...
    meta = {"region_id": region.region_id, "requests_made": 0, "url": overpass_url, "errors": []}
    if requests is None:
        meta["errors"].append("requests not installed")
        return [], meta

    if use_area_query:
        # Try to query by region_name as an administrative boundary
    ...
    try:
        ...
        for el in data.get("elements", []):
            ...
            if (plat is None or plon is None) and isinstance(el.get("center"), dict):
                plat = el["center"].get("lat"); plon = el["center"].get("lon")
            out.append({
...
def save_raw_for_region(raw_list: List[Dict[str, Any]], basepath: str, region_id: str) -> Tuple[str, str]:
    ...
    cols = [
        "place_id","name","source","lang","cell_idx","query_center_lat","query_center_lon",
        "lat","lon","page","region_id","region_name","collected_at"
    ]
...
def parse_locations_csv(path: str, *, gmaps_client: Optional["googlemaps.Client"], default_radius_km: float, default_grid_step_km: float) -> List[Region]:
    out: List[Region] = []
    with open(path, "r", encoding="utf-8-sig") as f:
        ...
        for i, row in enumerate(reader):
            ...
            lat = row.get("center_lat"); lon = row.get("center_lon")
            if city:
                out.append(Region.from_city(rid, rname, city, radius_km=radius_km, grid_step_km=grid_step_km, gmaps_client=gmaps_client))
            elif lat and lon:
                out.append(Region(rid, rname, float(lat), float(lon), radius_km, grid_step_km))
            else:
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect restaurant names (Google Places + OSM) for one or more regions")
    # Regions
    parser.add_argument("--location", action="append", nargs=5, metavar=("region_id","region_name","center_lat","center_lon","radius_km"), help="Add a region by center/radius (can be repeated)")
    parser.add_argument("--locations-csv", type=str, default=None, help="CSV with region_id,region_name,city,center_lat,center_lon,radius_km,grid_step_km")
    parser.add_argument("--city", action="append", nargs=3, metavar=("region_id","region_name","city_name"), help="Add a region by city name (requires Google geocoding")
    ...
    parser.add_argument("--languages", nargs="*", default=["kn","en"], help="Languages to query, e.g., kn en")
    ...
    parser.add_argument("--overpass-url", type=str, default="https://overpass-api.de/api/interpreter")

    # Output
    parser.add_argument("--basepath", type=str, default=f"restaurants_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    if argv is None and in_ipykernel:
        argv = []
        extra = os.environ.get("COLLECT_ARGS");
        if extra:
...
def collect_run_notebook(regions: List[Dict[str, Any]], *,
                         use_google_places: bool = True,
                         google_api_key: Optional[str] = None,
                         languages: Iterable[str] = ("kn","en"),
                         grid_step_km: float = 2.0,
                         ...
                         overpass_url: str = "https://overpass-api.de/api/interpreter",
                         basepath: str = f"restaurants_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                         verbose: bool = True) -> List[str]:
    """Collect across many regions from a notebook. Returns list of JSONL paths."""
    gmaps_client = googlemaps.Client(key=google_api_key) if (use_google_places and google_api_key and googlemaps) else None
    out_paths: List[str] = []
    for reg in regions:
        if "city" in reg and (reg.get("center_lat") is None or reg.get("center_lon") is None):
            r = Region.from_city(
                reg["region_id"], reg.get("region_name") or reg["city"], reg["city"],
                radius_km=float(reg.get("radius_km", 20.0)), grid_step_km=float(reg.get("grid_step_km", grid_step_km)),
                gmaps_client=gmaps_client,
            )
        else:
            r = Region(
                region_id=reg["region_id"], region_name=reg.get("region_name", reg["region_id"]),
                center_lat=float(reg["center_lat"]), center_lon=float(reg["center_lon"]),
                radius_km=float(reg.get("radius_km", 20.0)), grid_step_km=float(reg.get("grid_step_km", grid_step_km)),
            )
        raw_all: List[Dict[str, Any]] = []
        if use_google_places:
            raw_p, meta_p = collect_places_for_region(
                r, gmaps_client=gmaps_client, languages=tuple(languages),
                places_radius_m=places_radius_m, places_sleep_s=places_sleep_s,
                max_cells=max_cells, verbose=verbose,
            )
            # Save meta
            meta_p.update({"region_name": r.region_name})
            save_meta(meta_p, basepath, r.region_id)
            raw_all += raw_p
        if use_overpass:
            raw_o, meta_o = collect_overpass_for_region(r, overpass_url=overpass_url, verbose=verbose)
            meta_o.update({"region_name": r.region_name})
            save_meta(meta_o, basepath, r.region_id + "_osm")
            raw_all += raw_o
        p_jsonl, p_csv = save_raw_for_region(raw_all, basepath, r.region_id)
        out_paths.append(p_jsonl)
    return out_paths
...
def main() -> None:
    ...
    # Build regions list
    gmaps_client = googlemaps.Client(key=args.google_api_key) if (args.use_google_places and args.google_api_key and googlemaps) else None
    regions: List[Region] = []
    if args.locations_csv:
        regions += parse_locations_csv(args.locations_csv, gmaps_client=gmaps_client, default_radius_km=20.0, default_grid_step_km=args.grid_step_km)
    if args.location:
        for rid, rname, lat, lon, rad in args.location:
            regions.append(Region(rid, rname, float(lat), float(lon), float(rad), grid_step_km=args.grid_step_km))
    if args.city:
        for rid, rname, city in args.city:
            regions.append(Region.from_city(rid, rname, city, radius_km=20.0, grid_step_km=args.grid_step_km, gmaps_client=gmaps_client))

    if not regions:
        # default: Bengaluru center
        regions = [Region("blr","Bengaluru", 12.9716, 77.5946, 22.0, grid_step_km=args.grid_step_km)]
    ...
    for r in regions:
        ...
        if args.use_google_places:
            raw_p, meta_p = collect_places_for_region(
                r, gmaps_client=gmaps_client, languages=languages,
                places_radius_m=args.places_radius_m, places_sleep_s=args.places_sleep_s,
                max_cells=args.max_cells, verbose=args.verbose,
            )
            meta_p.update({"region_name": r.region_name})
            save_meta(meta_p, args.basepath, r.region_id)
            raw_all += raw_p
        if args.use_overpass:
            raw_o, meta_o = collect_overpass_for_region(r, overpass_url=args.overpass_url, verbose=args.verbose)
            meta_o.update({"region_name": r.region_name})
